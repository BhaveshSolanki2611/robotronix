...
interface Submission { id: string; name: string; email: string; company?: string | null; service?: string | null; message?: string; status: string; createdAt: string; }
interface Demo { id: string; name: string; email: string; company: string; demoType: string; date?: string | null; status: string; createdAt: string; }
...
interface AdminDashboardClientProps extends AdminDashboardData {
...
export default function AdminDashboardClient({
  ...
  const [contacts, setContacts] = useState<Submission[]>(initialContacts);
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "var(--bg-primary)" }}>
        <div className="w-full max-w-sm rounded-2xl p-8" style={cardStyle}>
          <div className="text-center mb-6">
            ...
            <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Enter admin credentials to continue</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl px-4 py-3 text-sm outline-none" style={{ ...cardStyle, color: "var(--text-primary)" }} placeholder="Admin email" autoComplete="username" autoFocus />
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-xl px-4 py-3 text-sm outline-none" style={{ ...cardStyle, color: "var(--text-primary)" }} placeholder="Admin password" autoComplete="current-password" />
